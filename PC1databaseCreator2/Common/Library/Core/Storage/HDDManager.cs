...
namespace PC1databaseCreator.Core.Storage
{
    public sealed class HDDManager : StorageManager
    {
        ...
        private StorageDriveInfo SelectOptimalDrive(long dataSize)
        {
            lock (_driveLock)
            {
                foreach (var drive in _config.PrimaryDrives)
                {
                    var systemDriveInfo = new System.IO.DriveInfo(Path.GetPathRoot(drive.Path));
                    if (systemDriveInfo.AvailableFreeSpace > dataSize + (MIN_FREE_SPACE_GB * 1024L * 1024L * 1024L))
                    {
                        return new StorageDriveInfo(drive.Path);
                    }
                }
                return null;
            }
